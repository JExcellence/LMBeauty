...
export const WimpernliftingGallery: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInstagram = async () => {
            try {
                // Use the Next.js API route (same as ServicesSection)
                ...
                if (result.success && result.data && Object.keys(result.data).length > 0) {
                    const liftingPosts: InstagramPost[] = [];
                    
                    // Check for "lifting" category from backend (same as ServicesSection approach)
                    if (result.data.lifting && Array.isArray(result.data.lifting)) {
                        console.log('Found lifting category with', result.data.lifting.length, 'posts');
                        result.data.lifting.forEach((post: any) => {
                            liftingPosts.push({
                                id: post.id,
                        ...
                        console.log('No lifting category found. Available categories:', Object.keys(result.data));
                    }

                    console.log('Setting', liftingPosts.length, 'lifting posts');
                    setPosts(liftingPosts.slice(0, 6));
                } else {
            ...
            <Column fillWidth maxWidth={80} gap="xl" paddingX="xl" s={{maxWidth: 100, paddingX: "l"}}>
                <Column gap="xs" center>
                    <Heading as="h2" variant="display-strong-xs" align="center">
                        Ergebnisse <Text as="span" onBackground="brand-weak">#WimpernliftingOldenburg</Text>
                    </Heading>
                    ...
                    {posts.length > 0 ? (
                        posts.map((post, index) => (
                            ...
                            >
                                <img
                                    src={post.media_url}
                                    alt={post.caption || `Wimpernlifting Ergebnis ${index + 1}`}
                                    loading="lazy"
                                />
...
export default WimpernliftingGallery;
